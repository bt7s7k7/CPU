...
function drawCPU(state, element) {
	...
	var createIndicator = (color, enabled, parent) => {
		...
		indicator.style.backgroundColor = color.mul(enabled ? 1 : 0.5).toHex()
	...
	state.components.toArray().forEach(v => {
		...
		if ("reset" in v.value) createIndicator([127, 0, 127], v.value.reset, header)
