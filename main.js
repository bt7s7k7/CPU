	...
	for (var i = beginLine; i < endLine; i++) {
		...
		if (i < debugDatabase.codeEnd) {
			if (argCountDown.length > 0) {
				...
				if (value in debugDatabase.points && i != value) {
					text += "; *" + debugDatabase.points[value]
				}
			} else {
			...
			if (i in debugDatabase.points) {
				text += "; " + debugDatabase.points[i]
			}

		} else {
			if (lastValue) {
				lastValue.size--
				if (lastValue.size <= 0) lastValue = null
				text += "; ^"
			} else {
